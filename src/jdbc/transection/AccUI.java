	...
	public void actionPerformed( ActionEvent ev ){
		...
		if ( o == b_moveMoney )
		{
			try{
				...
				String 	recAccNum = tf_recvAccNum.getText();
				...
				int result = logic.moveAccount(sendAccNum, recAccNum, moveMoney);
				//				- 1번의 입력값을 인자로 넘김
				if(result == 0) JOptionPane.showMessageDialog(null, "이체성공"); // - 호출 후 넘겨받는 리턴값으로는 0이면 정상처리
				else if(result == -1) JOptionPane.showMessageDialog(null, "이체실패"); //	 -1이면 잘못된 경우이므로 메세지박스 출력
